 ...
 public:
  ...
  DecodeResult Decode(bool aIsMetadataDecode) override {
    ...
    mDecodedData.emplace(Dav1dPictureToYCbCrAData(
        ...
        mAlphaPlane.isSome() ? parsedImg->premultiplied_alpha : false));
 ...
 public:
  ...
  DecodeResult Decode(bool aIsMetadataDecode) override {
    ...
    mDecodedData.emplace(AOMImageToYCbCrAData(
        ...
        mOwnedAlphaPlane ? parsedImg->premultiplied_alpha : false));
...
layers::PlanarYCbCrAData AOMDecoder::AOMImageToYCbCrAData(
  ...
  if (aAlphaPlane) {
    ...
    data.mAlphaColorDepth = ColorDepthForBitDepth(aAlphaPlane->bit_depth);
    data.mPremultipliedAlpha = aPremultipliedAlpha;
  }
...
nsAVIFDecoder::DecodeResult nsAVIFDecoder::Decode(
  ...
  // Chech the alpha data is valid before processing it.
  if (decodedData.hasAlpha()) {
    if (decodedData.mAlphaSize != decodedData.mYSize) {
      return AsVariant(NonDecoderResult::AlphaYSizeMismatch);
    }
    if (decodedData.mAlphaColorDepth != decodedData.mColorDepth) {
  ...
  const bool disablePremultiplyAlpha =
      bool(GetSurfaceFlags() & SurfaceFlags::NO_PREMULTIPLY_ALPHA);
  SurfacePipeFlags pipeFlags = SurfacePipeFlags();
  // If the image isn't premultiplied with alpha data, do it in our pipeline.
  if (!disablePremultiplyAlpha && hasAlpha &&
