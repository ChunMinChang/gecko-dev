...
struct YUV8BitData {
  explicit YUV8BitData(const layers::PlanarYCbCrData& aData) {
    ...
    mData.mYChannel = yChannel.get();

    int bitDepth = BitDepthForColorDepth(aData.mColorDepth);

    ConvertYCbCr16to8Line(mData.mYChannel,
                          mData.mYStride,
    ...
    if (cbcrMemorySize) {
      ...
      mData.mCbChannel = cbChannel.get();
      mData.mCrChannel = crChannel.get();

      ConvertYCbCr16to8Line(mData.mCbChannel,
                            mData.mCbCrStride,
      ...
      ConvertYCbCr16to8Line(mData.mCrChannel,
                            mData.mCbCrStride,
  ...
  const layers::PlanarYCbCrData& Get8BitData() {
    return mData;
  }
                          ...
                          int32_t aStride)
{
  ...
  if (aDestSize != srcData.mPictureRect.Size()) {
    ...
    ConvertYCbCrToRGB(srcData, aDestFormat, aDestSize, aDestBuffer, aStride,
                      yuvtype);
  }
}
