...
/*
 * The below are helpers to read or write data from or to ArrayBuffer or
 * ArrayBufferView.
 */
...
template <class T>
static void WriteData(const T& aBuffer, size_t aOffset, const uint8_t* aSrc,
                      size_t aNElem) {
  PodCopy(aBuffer.Data() + aOffset, aSrc, aNElem);
}

static void WriteData(
    const MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer& aBuffer,
    size_t aOffset, const uint8_t* aSrc, size_t aNElem) {
  if (aBuffer.IsArrayBufferView()) {
    WriteData(aBuffer.GetAsArrayBufferView(), aOffset, aSrc, aNElem);
    return;
  }

  MOZ_ASSERT(aBuffer.IsArrayBuffer());
  WriteData(aBuffer.GetAsArrayBuffer(), aOffset, aSrc, aNElem);
}
...
static Maybe<nsAutoCString> ComputeLayoutAndAllocationSize(
  ...
  for (size_t i = 0; i < planes.Length(); ++i) {
    ...
    const uint32_t sampleWidthBytes =
        static_cast<uint32_t>(sampleSize.Width()) * aFormat.SampleBytes(p);

    ComputedPlaneLayout layout{
        .mDestinationOffset = 0,
        .mDestinationStride = 0,
        .mSourceTop = static_cast<uint32_t>(aRect.Y() / sampleSize.Height()),
        .mSourceHeight =
            static_cast<uint32_t>(aRect.Height() / sampleSize.Height()),
        .mSourceLeftBytes = static_cast<uint32_t>(aRect.X()) / sampleWidthBytes,
        .mSourceWidthBytes =
            static_cast<uint32_t>(aRect.Width()) / sampleWidthBytes};
    if (aPlaneLayouts) {
      const PlaneLayout& p = aPlaneLayouts->ElementAt(i);
      if (p.mStride < layout.mSourceWidthBytes) {
        return Some(
            nsAutoCString("Mismatch between layout and parsed rect info"));
      }
      layout.mDestinationOffset = p.mOffset;
      layout.mDestinationStride = p.mStride;
    } else {
...
static Maybe<nsAutoCString> ParseVideoFrameCopyToOptions(
  ...
  if (aOptions.mRect.WasPassed()) {
    // TODO: spec doesn't check if any attribute of rect is negative, but we
    // check this as what `Validate VideoFrameInit` does. Is that right?
    if (Maybe<nsAutoCString> error =
...
static already_AddRefed<gfx::DataSourceSurface> ToBGRASurface(
    gfx::DataSourceSurface* aSurface) {
  MOZ_ASSERT(aSurface);

  if (aSurface->GetFormat() == gfx::SurfaceFormat::B8G8R8A8) {
    RefPtr<gfx::DataSourceSurface> bgraSurface = aSurface;
    return bgraSurface.forget();
  }

  // TODO: Still need for SurfaceFormat::BGRX?
...
static already_AddRefed<layers::Image> CreateImageFromRawData(
  ...
  // TODO: MOZ_ASSERT(NS_IsMainThread()) ?
  ...
  // Gecko has favor of BGRA so we convert surface into BGRA format first.
  RefPtr<gfx::DataSourceSurface> bgraSurface = ToBGRASurface(surface);
  if (NS_WARN_IF(!bgraSurface)) {
    return nullptr;
  }

  // Create an Image from the BGRA SourceSurface like what ImageBitmap does.
  RefPtr<layers::SourceSurfaceImage> image =
      new layers::SourceSurfaceImage(bgraSurface->GetSize(), bgraSurface);
  return image.forget();
}
...
static already_AddRefed<VideoFrame> CreateVideoFrameFromBuffer(
  ...
  if (aInit.mVisibleRect.WasPassed()) {
    ...
    // we check this as what `Validate VideoFrameInit` does. Is that right?
    if (Maybe<nsAutoCString> error =
  ...
  // TODO: Check range of aInit.mCoded{Width, Height}?
  const gfx::IntSize codeSize(aInit.mCodedWidth, aInit.mCodedHeight);
  gfx::IntRect parsedRect({0, 0}, codeSize);
  if (Maybe<nsAutoCString> error =
          ParseVisibleRect(parsedRect, visibleRect, codeSize, format)) {
    aRv.ThrowTypeError(*error);
    return nullptr;
  }
  ...
  // TODO: If only one of displayWidth or displayHeight exists, `Validate
  // VideoFrameInit` says it's invalid, but it's ok here?
  const uint32_t displayWidth = aInit.mDisplayWidth.WasPassed()
                                    ? aInit.mDisplayWidth.Value()
                                    : static_cast<uint32_t>(parsedRect.Width());
  const uint32_t displayHeight =
      aInit.mDisplayHeight.WasPassed()
          ? aInit.mDisplayHeight.Value()
          : static_cast<uint32_t>(parsedRect.Height());
  ...
  MOZ_ASSERT(data->GetSize() == codeSize);

  // TODO: Spec doesn't assign aInit.mFormat to inner format value.
  return MakeAndAddRef<VideoFrame>(
      global, data.forget(), aInit.mFormat, codeSize, parsedRect,
...
already_AddRefed<VideoFrame> VideoFrame::Constructor(
    ...
    const VideoFrameBufferInit& aInit, ErrorResult& aRv) {
  return CreateVideoFrameFromBuffer(aGlobal, aData, aInit, aRv);
}
...
Nullable<VideoPixelFormat> VideoFrame::GetFormat() const {
  // TODO: Return Nullable<T>() if this is _detached_.
  return mResource
...
already_AddRefed<DOMRectReadOnly> VideoFrame::GetCodedRect() const {
  // TODO: Return nullptr if this is _detached_.
...
// TODO: https://w3c.github.io/webcodecs/#dom-videoframe-allocationsize
uint32_t VideoFrame::AllocationSize(const VideoFrameCopyToOptions& aOptions,
                                    ErrorResult& aRv) {
  // TODO: aRv.Throw(NS_ERROR_DOM_INVALID_STATE_ERR) if this is _detached_.
  if (!mResource) {
    aRv.Throw(NS_ERROR_DOM_NOT_SUPPORTED_ERR);
    return 0;
  }
...
already_AddRefed<Promise> VideoFrame::CopyTo(
  ...
  // TODO: spec says "Let numPlanes be the number of planes as defined by
  // [[format]]" but "numPlanes" is never used?
  nsTArray<Format::Plane> planes = mResource->Format().Planes();
  MOZ_ASSERT(layout.mComputedLayouts.Length() == planes.Length());

  // TODO: Do copy in thread pool?
  for (size_t i = 0; i < layout.mComputedLayouts.Length(); ++i) {
    uint32_t sourceStride = mResource->Stride(planes[i]);
    ComputedPlaneLayout& l = layout.mComputedLayouts[i];
    uint32_t sourceOffset = l.mSourceTop * sourceStride;
    sourceOffset += l.mSourceLeftBytes;
    uint32_t destinationOffset = l.mDestinationOffset;

    for (uint32_t row = 0; row < l.mSourceHeight; ++row) {
      WriteData(aDestination, static_cast<size_t>(destinationOffset),
                mResource->Data() + sourceOffset,
                static_cast<size_t>(l.mSourceWidthBytes));
      sourceOffset += sourceStride;
      destinationOffset += l.mDestinationStride;
    }
  }

  // TODO: Spec doesn't specify the resolved value.
  p->MaybeResolve(true);
  return p.forget();
}
 ...
 * bits. The pixel layouts for a 4 x 2 image are illustrated below:
 ...
 *  Y: Y1 Y2 Y3 Y4 ^ height
 *     Y5 Y6 Y7 Y8 v
 * UV: U1 V1 U2 V2 => Y's width, 1/2 Y's Y's height
 *
...
const uint8_t* VideoFrame::Resource::Data() const {
  return reinterpret_cast<uint8_t*>(mImage->GetImplData());
}
  ...
  switch (aPlane) {
    ...
    case Format::Plane::A:
      switch (mFormat.PixelFormat()) {
        ...
        case VideoPixelFormat::BGRA:
        case VideoPixelFormat::BGRX:
          return (width * BytesPerPixel(mFormat.ToSurfaceFormat())).value();
        default:
          MOZ_ASSERT_UNREACHABLE("invalid format");
          break;
      }
