...
class MediaTrackGraphImpl : public MediaTrackGraph,
 ...
 public:
  ...
  uint32_t AudioInputChannelCount() {
    ...
    for (const auto& listener : track->mDataUsers) {
  ...
  // Maps AudioDeviceID to a device track that track of their users (that are
  // listeners in AudioInputTrack). This is used to deliver audio input/output
  // frames and to notify the listeners that the audio device that delivers the
  // audio frames has changed. This is only touched on the graph thread. The
  // owner of the NativeInputTrack here is mDeviceTracks, which is only touched
  // by main thread.
