...
nsCString EncoderConfig::SampleFormat::ToString() const {
  nsAutoCString rv;
  rv.AppendPrintf("SampleFormat: %s", dom::GetEnumString(mPixelFormat).get());
  if (mColorRange == gfx::ColorRange::FULL) {
    rv.AppendLiteral(" (full-range)");
  } else {
    rv.AppendLiteral(" (limited-range)");
  }
  return rv;
}
...
EncoderConfig::SampleFormat::FromImage(layers::Image* aImage) {
  ...
  if (format.isNothing()) {
    return Err(MediaResult(
        ...
        nsPrintfCString("unsupported image format: %d", aImage->GetFormat())));
  }
