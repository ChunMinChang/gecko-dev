...
void MPRISServiceHandler::SetMediaMetadataInternal(
  ...
  if (aClearArtUrl) {
    mMPRISMetadata.mArtUrl = EmptyString();
  }
  EmitMetadataChanged();
}
  ...
  mImageFetcher->FetchImage()
      ->Then(
          ...
          [this, self](const nsCOMPtr<imgIContainer>& aImage) {
            ...
            // Assume the url specifying the image is secure if the image can be
            // fetched successfully
            if (SetImageToDisplay(mFetchingUrl)) {
              ...
              LOG("Failed to set image to MPRIS. Try next image");
              mCurrentImageUrl = EmptyString();
              LoadImageAtIndex(mNextImageIndex++);
            }
...
bool MPRISServiceHandler::SetImageToDisplay(const nsAString& aUrl) {
  mMPRISMetadata.mArtUrl = aUrl;
  if (!EmitMetadataChanged()) {
    mMPRISMetadata.mArtUrl = EmptyString();
    return false;
  }
  return true;
...
GVariant* MPRISServiceHandler::GetMetadataAsGVariant() const {
  ...
  if (!mMPRISMetadata.mArtUrl.IsEmpty()) {
    g_variant_builder_add(
        &builder, "{sv}", "mpris:artUrl",
        g_variant_new_string(static_cast<const gchar*>(
            NS_ConvertUTF16toUTF8(mMPRISMetadata.mArtUrl).get())));
  }
